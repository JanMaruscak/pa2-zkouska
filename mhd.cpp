    ...
    unsigned findCheapest(const string &from, const string &to){
        ...
        while(!pq.empty()){
            ...
            if(curr.first == to){
                ...
                if(prev.count(curr.first) == 0 || prev[curr.first].second > curr.second)
                    prev[curr.first].second = curr.second;

            }
            for (const auto &item: graph[curr.first]){
                if(prev.count(item.first) != 0 && prev[item.first].second <= item.second+ curr.second){
                    continue;
                }
