...
public:
    ...
    map<string, string> colonise(const vector<Cbase> & bases) const {
        ...
        // { {sector, species}, {time, when} }

        map<string, pair<pair<string, int>, bool>> visited;
        // sector -> < <species, time> bool>
        ...
        while(!q.empty()){
            ...
            if(it != graph.end())
                for (const auto &n: it->second){
                    auto f = visited.find(n);
                    if(f == visited.end() && visited[current.first.first].second){
                        q.push({{n, current.first.second}, {current.second.first+1, current.second.second}});
                        visited[n] = {{current.first.second, current.second.first+1}, true};
                    }
                    else if(f != visited.end() && f->second.first.first != current.first.second && f->second.first.second == current.second.first+1){
